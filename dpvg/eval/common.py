...
from dpvg.envs.voting_game import get_step_detail
...
class Simulator():
    ...
    def rollout_and_save(self, policy: TensorDictModule, n_episodes: int, path: str):
        ...
        for _ in tqdm(range(n_episodes // self.n_envs + 1), position=0):
            self.env.reset()
            episodes = self.env.rollout(
            ...
            for i in tqdm(range(episodes.shape[0]), position=1, leave=False):
                ...
                ep_collection.append(ep_detail)
