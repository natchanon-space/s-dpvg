...
class IQL(MarlAlgorithm):
    ...
    def build(self):
        ...
        value_net = MultiAgentMLP(
            n_agent_inputs=self.env.observation_spec["agents", "observation"].shape[-1],
            n_agent_outputs=1,  # one for each choice
            n_agents=self.n_agents,
            centralized=False,
            share_params=False,
            device=self.cfg.device,
    ...
    def train(self, logger = None, checkpoint_iter = None):
        ...
        ## loss module
        self.loss_module = DiscreteIQLLoss(
            self.policy,
            self.qvalue,
            self.value,
        ...
        self.loss_module.set_keys(
            ...
            state_action_value=("agents", "action_value"),
            done=("agents", "done"),
    ...
    def save(self, log_dir, suffix = None):
        if suffix:
            ...
            value_path = os.path.join(log_dir, f"checkpoint_value_{suffix}.pt2")
